...
namespace nanoFramework.Tools.FirmwareFlasher
{
    ...
    {
        private readonly CmsisDap _dap;
        ...
        /// <summary>
        /// This property is <see langword="true"/> if a CMSIS-DAP probe is connected.
        /// </summary>
        public bool DevicePresent => !string.IsNullOrEmpty(ProbeId);

        /// <summary>
        /// Serial number of the connected CMSIS-DAP probe.
        /// </summary>
        public string ProbeId { get; private set; }

        /// <summary>
        /// Product name of the connected CMSIS-DAP probe.
        /// </summary>
        ...
        /// <summary>
        /// Creates a new <see cref="StmSwdDevice"/> and connects via a CMSIS-DAP probe.
        /// If a probe serial number is provided it will try to connect to that probe.
        /// Otherwise connects to the first available CMSIS-DAP probe.
        /// </summary>
        ...
        public StmSwdDevice(string probeId = null)
        {
            var probes = CmsisDap.Enumerate();

            if (probes.Count == 0)
            {
                throw new CantConnectToJtagDeviceException(
                    "No CMSIS-DAP debug probes found. Make sure a SWD probe is connected. " +
                    "On Linux, you may need a udev rule for HID access: " +
                    ...
                    "On macOS, no additional drivers are needed (IOKit is built-in).");
            }
            ...
            else
            {
                ...
                {
                    throw new CantConnectToJtagDeviceException(
                        $"CMSIS-DAP probe with serial '{probeId}' not found.");
                }
            }
            ...
            catch (SwdProtocolException ex)
            {
                Dispose();
                throw new CantConnectToJtagDeviceException(
                    $"Failed to connect to target via SWD: {ex.Message}");
            }
            catch (Exception ex) when (!(ex is CantConnectToJtagDeviceException))
            {
                Dispose();
                throw new CantConnectToJtagDeviceException(
                    $"Failed to initialize CMSIS-DAP connection: {ex.Message}");
            }
        }

        /// <summary>
        /// Lists connected CMSIS-DAP debug probes.
        /// </summary>
        ...
        public override string ToString()
        {
            ...
            deviceInfo.AppendLine("Interface: Native SWD (CMSIS-DAP)");
