...
### Advanced: PICOBOOT direct flash

nanoff supports the **PICOBOOT** native USB protocol for direct flash operations. This bypasses the UF2 mass storage path and communicates directly with the RP2040/RP2350 ROM bootloader. It requires the device to be in BOOTSEL mode and uses `LibUsbDotNet` for USB communication.

To update firmware using the PICOBOOT protocol (direct binary flash, no UF2 conversion):

```console
nanoff --update --target RASPBERRY_PI_PICO --picoboot
```

To update firmware and verify the flash contents after writing:

```console
nanoff --update --target RASPBERRY_PI_PICO --picoboot --verify
```

To force a running device into BOOTSEL mode before flashing (requires nanoFramework USB stdio support in the running firmware):

```console
nanoff --update --target RASPBERRY_PI_PICO --forcebootsel
```

To erase all flash contents on the device:

```console
nanoff --platform rpi_pico --picoboot --masserase
```

To back up the current flash contents to a file:

```console
nanoff --platform rpi_pico --picoboot --readflash backup.bin
```

To flash with a custom CLR binary file (instead of downloading from Cloudsmith):

```console
nanoff --update --target RASPBERRY_PI_PICO --clrfile path/to/nanoCLR.bin
```

This also works with the PICOBOOT path:

```console
nanoff --update --target RASPBERRY_PI_PICO --picoboot --clrfile path/to/nanoCLR.bin --verify
```

To flash raw binary files at specific addresses via PICOBOOT (advanced):

```console
nanoff --platform rpi_pico --picoboot --binfile firmware.bin --address 0x10000000
```

Multiple files can be flashed at different addresses with optional verification:

```console
nanoff --platform rpi_pico --picoboot --binfile bootloader.bin --address 0x10000000 --binfile app.bin --address 0x10010000 --verify
```

### Advanced: RP2350 device info and OTP (PICOBOOT)

On RP2350 devices, nanoff can query extended device information including flash JEDEC ID, secure boot status, and OTP configuration via the PICOBOOT protocol.

To show extended device details for an RP2350 in BOOTSEL mode:

```console
nanoff --platform rpi_pico --devicedetails --picoboot
```

To dump the OTP (One-Time-Programmable) memory to a file for inspection (RP2350 only):

```console
nanoff --platform rpi_pico --picoboot --otpdump otp.bin
```

> **Note:** The `--picoboot` option requires appropriate USB drivers (WinUSB on Windows, libusb on Linux/macOS). On Linux, udev rules may be required for non-root access: `SUBSYSTEM=="usb", ATTR{idVendor}=="2e8a", MODE="0666"`. The `--verify`, `--readflash`, and `--otpdump` options require `--picoboot`. OTP dump and extended device info are only available on RP2350 devices.
